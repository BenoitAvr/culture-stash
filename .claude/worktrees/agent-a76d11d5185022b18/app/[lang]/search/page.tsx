...
export default async function SearchPage({
  ...
  if (!query) {
    return (
      <div style={{ maxWidth: 800, margin: '0 auto', padding: '60px 24px', textAlign: 'center' }}>
        <p style={{ color: '#777' }}>{t.search.enter}</p>
      </div>
    )
  }
  ...
  return (
    <div style={{ maxWidth: 800, margin: '0 auto', padding: '40px 24px' }}>
      <p style={{ color: '#777', fontSize: 13, marginBottom: 32 }}>
        {total} {total !== 1 ? t.search.results : t.search.result} {t.search.for}{' '}
        <span style={{ color: '#f0f0f0' }}>&ldquo;{query}&rdquo;</span>
      </p>

      {topics.length > 0 && (
        <section style={{ marginBottom: 40 }}>
          <h2 style={{ fontSize: 11, textTransform: 'uppercase', letterSpacing: '.08em', color: '#777', marginBottom: 14 }}>{t.search.topics}</h2>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {topics.map(topic => {
              const tr = topic.translations[0]
              return (
                <Link key={topic.id} href={`/${lang}/topics/${topic.slug}`} style={{ textDecoration: 'none' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 14, background: '#161616', border: '1px solid #2a2a2a', borderRadius: 12, padding: '14px 18px' }}>
                    <span style={{ fontSize: 26 }}>{topic.emoji}</span>
                    <div>
                      <div style={{ fontWeight: 500, fontSize: 15, color: '#f0f0f0', marginBottom: 2 }}>{tr?.title ?? topic.title}</div>
                      <div style={{ color: '#777', fontSize: 12 }}>{tr?.desc ?? topic.desc}</div>
                    </div>
                  </div>
                </Link>
              )
      ...
      {resources.length > 0 && (
        <section>
          <h2 style={{ fontSize: 11, textTransform: 'uppercase', letterSpacing: '.08em', color: '#777', marginBottom: 14 }}>{t.search.resources}</h2>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {resources.map(r => {
              ...
              return (
                <Link key={r.id} href={`/${lang}/topics/${r.topic.slug}`} style={{ textDecoration: 'none' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 14, background: '#161616', border: '1px solid #2a2a2a', borderRadius: 12, padding: '14px 18px' }}>
                    ...
                    <div style={{ flex: 1 }}>
                      <div style={{ fontWeight: 500, fontSize: 14, color: '#f0f0f0', marginBottom: 2 }}>
                        {tr?.title ?? r.title}
                      ...
                      <div style={{ color: '#777', fontSize: 12 }}>{tr?.sub ?? r.sub}</div>
                    </div>
                    <div style={{ fontSize: 12, color: '#555' }}>{t.search.in} <span style={{ color: '#777' }}>{r.topic.title}</span></div>
                  </div>
                </Link>
              )
            })}
      ...
      {total === 0 && (
        <p style={{ color: '#777', padding: '40px 0', textAlign: 'center' }}>{t.search.noResults}</p>
      )}
    </div>
  )
}
