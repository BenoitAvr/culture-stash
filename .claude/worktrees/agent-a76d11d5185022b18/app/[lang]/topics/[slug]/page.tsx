  ...
  if (lang !== 'fr' && !tr) {
    return (
      <div style={{ maxWidth: 700, margin: '0 auto', padding: '80px 24px', textAlign: 'center' }}>
        ...
        <h1 style={{ fontFamily: "'Fraunces', serif", fontSize: 28, fontWeight: 900, color: '#f0f0f0', margin: '24px 0 12px' }}>
          {lang === 'en' ? 'Not available in English yet' : 'Pas encore disponible dans cette langue'}
        </h1>
        <p style={{ color: '#777', fontSize: 14, lineHeight: 1.7, marginBottom: 32, maxWidth: 400, margin: '0 auto 32px' }}>
          {lang === 'en'
        ...
        <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap' }}>
          <Link href={`/fr/topics/${slug}`} style={{ padding: '10px 20px', borderRadius: 8, border: '1px solid #2a2a2a', color: '#aaa', fontSize: 14, textDecoration: 'none' }}>
            {lang === 'en' ? 'Read in French' : 'Lire en français'} →
          </Link>
          {session && (
            <Link href={`/${lang}/topics/${slug}/translate`} style={{ padding: '10px 20px', borderRadius: 8, border: 'none', background: '#c8f55a', color: '#0e0e0e', fontSize: 14, fontWeight: 600, textDecoration: 'none' }}>
      ...
      <div style={{ padding: '28px 0 24px', borderBottom: '1px solid #2a2a2a' }}>
        ...
        {(topic.parent || session) && (
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
            {topic.parent ? (
              <Link href={`/${lang}/topics/${topic.parent.slug}`} style={{ fontSize: 12, color: '#555', textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: 5 }}>
                ← {topic.parent.emoji} {parentTitle}
              </Link>
            ) : <span />}
            {session && (
              <Link href={`/${lang}/topics/${slug}/edit`} style={{ fontSize: 12, color: '#444', textDecoration: 'none', border: '1px solid #2a2a2a', borderRadius: 6, padding: '4px 10px' }}>
                Modifier
              </Link>
            )}
          </div>
        ...
        <h1 style={{ fontFamily: "'Fraunces', serif", fontSize: 40, fontWeight: 900, letterSpacing: -1, lineHeight: 1.1, marginBottom: 12, color: '#f0f0f0' }}>
          {title}
        </h1>

        {/* Description */}
        <p style={{ color: '#aaa', maxWidth: 580, lineHeight: 1.6, margin: 0 }}>{desc}</p>

        {/* Sous-sujets */}
        {(children.length > 0 || session) && (
          <div style={{ marginTop: 20 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 10 }}>
              <p style={{ fontSize: 11, textTransform: 'uppercase', letterSpacing: '.08em', color: '#444', margin: 0 }}>Sous-sujets</p>
              {session && (
                <Link href={`/${lang}/topics/new?parent=${topic.id}`} style={{ fontSize: 11, color: '#555', textDecoration: 'none', border: '1px dashed #2a2a2a', borderRadius: 5, padding: '2px 8px' }}>
                  + Ajouter
                </Link>
              )}
            </div>
            <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
              {children.map(c => (
                <Link key={c.slug} href={`/${lang}/topics/${c.slug}`} style={{ textDecoration: 'none', display: 'flex', alignItems: 'center', gap: 8, padding: '8px 14px', background: '#161616', border: '1px solid #2a2a2a', borderRadius: 10 }}>
                  <span style={{ fontSize: 20 }}>{c.emoji}</span>
                  <div>
                    <div style={{ fontSize: 13, fontWeight: 500, color: '#f0f0f0' }}>{c.title}</div>
                    {c.desc && <div style={{ fontSize: 11, color: '#555', marginTop: 1, maxWidth: 180, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{c.desc}</div>}
                  </div>
                </Link>
              ))}
            </div>
