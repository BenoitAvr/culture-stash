      ...
      <div style={{ display: 'flex', gap: 2, opacity: isPending ? 0.6 : 1 }} onMouseLeave={() => setHover(0)}>
        {[1, 2, 3, 4, 5].map(star => (
          <button key={star} onClick={() => handleRate(star)} onMouseEnter={() => isLoggedIn ? setHover(star) : undefined} disabled={isPending}
            ...
            style={{ background: 'none', border: 'none', fontSize: 18, lineHeight: 1, padding: '0 1px', cursor: isLoggedIn ? 'pointer' : 'default', color: star <= display ? '#f5a623' : '#333', transition: 'color .1s' }}
          >
            {star <= display ? '★' : '☆'}
          </button>
        ))}
