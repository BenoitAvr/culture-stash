    ...
    <div style={{ width: '100%', maxWidth: 400, margin: '0 auto' }}>
      ...
      <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 12, boxShadow: 'var(--shadow)', padding: 28 }}>
        ...
        <form action={action} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div>
            <label style={{ display: 'block', fontSize: 12, color: 'var(--fg-3)', marginBottom: 6, textTransform: 'uppercase', letterSpacing: '.06em' }}>{t.nameLabel}</label>
            <input name="name" type="text" required placeholder="Your name" style={{ width: '100%', background: 'var(--bg-input)', border: '1px solid var(--border)', borderRadius: 8, padding: '10px 14px', color: 'var(--fg)', fontSize: 14, outline: 'none', fontFamily: 'inherit' }} />
          </div>
          <div>
          ...
          <div>
            ...
            <input name="password" type="password" required placeholder="Min. 6 chars" style={{ width: '100%', background: 'var(--bg-input)', border: '1px solid var(--border)', borderRadius: 8, padding: '10px 14px', color: 'var(--fg)', fontSize: 14, outline: 'none', fontFamily: 'inherit' }} />
          </div>
        ...
        <p style={{ textAlign: 'center', marginTop: 20, fontSize: 13, color: 'var(--fg-6)' }}>
          {t.hasAccount}{' '}
          <Link href={`/${lang}/auth/login`} style={{ color: 'var(--accent-fg)', textDecoration: 'none' }}>{t.loginLink}</Link>
        </p>
      </div>
    </div>
  )
